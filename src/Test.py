...
res = tn.parse(target=u'一个星期后') 
print(res)

res = tn.parse(target=u'一个月') 
print(res)

text = u'三个月五天后的12点半'.split(u'后')

res = tn.parse(target=text[0]) 
print(res)

month = json.loads(res)['timedelta']['month']
days = json.loads(res)['timedelta']['day']

res = tn.parse(target=text[1]) 
print(res)

res = tn.parse(target=u'两个小时后') 
print(res)

res = tn.parse(target=u'五日11点') 
print(res)
